...
export class ItemsService {
  ...
  catagories: string[] = ['food', 'drink', 'electronics', 'health'];
  ...
  public readonly filteredItems$ = combineLatest([
    ...
    map(
      ([loadedItems, searchQuery, selectedCatagory]: [
        Item[],
        string,
        string
      ]) => {
        ...
        const filteredItems: Item[] = loadedItems.filter((item) => {
          ...
          //   'searchInput': 'combined input',
          //   event: 'combinedInput',
          //   eventCategory:'combinedSearch',
          //   eventAction: searchQuery+ selectedCatagory})
          //   console.log(window.dataLayer)
          return (
            item.name.toLowerCase().includes(formattedSearchQuery) &&
            item.catagory.toLowerCase().includes(selectedCatagory)
          );
